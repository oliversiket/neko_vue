<template>
  <div v-if="!this.loading" class="hello">
    <form  class="container d-flex flex-column col-lg-4">
        ...
        <select v-if="breeds.length" v-model="selected" class="select" @change="checkBreed">
            <option disabled value="">Please select one</option>
            <option v-for="breed in breeds" :key="breed.id">
                {{ breed.name }}
            </option>
        </select>
        <router-link to="/facts"><button type="button" class="btn btn-info m-3">Show me</button></router-link>
    </form>
